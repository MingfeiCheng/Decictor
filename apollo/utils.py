...
from typing import List, Set, Tuple, Union, Any, TypeVar
...
class PositionEstimate:
    ...
    def is_too_close(self, rhs, threshold=5.0) -> bool:
        """
        ...
        return adc1p.distance(adc2p) < threshold
...
def generate_adc_polygon(position: Point3D, theta: float) -> List[Point3D]:
    ...
    points = []
    half_w = APOLLO_WIDTH / 2.0
    front_l = APOLLO_LENGTH - APOLLO_BACK2CENTER
    back_l = -1 * APOLLO_BACK2CENTER
    sin_h = math.sin(theta)
    cos_h = math.cos(theta)
...
def dynamic_obstacle_location_to_obstacle(_id: int,
                                          speed: float,
                                          loc: Point3D,
                                          agent_type: AgentTypeClass,
                                          heading: float) -> Tuple[PerceptionObstacle, Polygon]:
    position = Point3D(x=loc.x,
                       y=loc.y, z=loc.z)
    velocity = Point3D(x=math.cos(heading) * speed,
                       y=math.sin(heading) * speed, z=0.0)

    polygon_point = generate_polygon(position, heading, agent_type.length, agent_type.width)
    ...
    obs = PerceptionObstacle(
        ...
        velocity=velocity,
        acceleration=Point3D(x=0, y=0, z=0),
        length=agent_type.length,
        width=agent_type.width,
        height=agent_type.height,
        type=PerceptionObstacle.VEHICLE,
        timestamp=time.time(),
        tracking_time=1.0,
        polygon_point=polygon_point
    )
    polygon = Polygon([[p.x, p.y] for p in polygon_point])
    return obs, polygon

def static_obstacle_location_to_obstacle(_id: int,
                                         loc: WaypointClass,
                                         agent_type: AgentTypeClass) -> Tuple[PerceptionObstacle, Polygon]:
    ...
    obj_length = agent_type.length
    obj_width = agent_type.width
    obj_height = agent_type.height

    polygon_point = generate_polygon(position, heading, obj_length, obj_width)
...
def localization_to_obstacle(_id: int, data: LocalizationEstimate) -> PerceptionObstacle:
    ...
    obs = PerceptionObstacle(
        ...
        length=APOLLO_LENGTH,
        width=APOLLO_WIDTH,
        height=APOLLO_HEIGHT,
        type=PerceptionObstacle.VEHICLE,
...
def construct_lane_boundary_linestring(lane_msg: Lane) -> Tuple[Union[LineString, Any], Union[LineString, Any]]:
    """
