...
class RegionCalculator:
    ...
    def __init__(self, cfg: DictConfig, adv_seed: SeedWrapper):
        ...
        self.threshold_ego = [0.0, 0.0] #cfg.get('threshold_ego', [0.0, 0.0])  # [delta_length, delta_width]
        self.threshold_static = [0.0, 0.0] #cfg.get('threshold_static', [0.0, 0.0])  # [delta_length, delta_width]
        self.threshold_vehicle = [0.0, 0.0] #cfg.get('threshold_vehicle', [0.0, 0.0])  # [delta_length, delta_width]
    ...
    def preprocess(self, source_seed: SeedWrapper, motion_interval: float):
        ...
        for i, t in enumerate(self.time_list[:-1]):
            ...
            for npc_id in npc_ids:
                if npc_id == 'ego':
                    prohibit_region = self._prohibited_region_ego(self.adv_record, 0.0, 0.3)
                else:
                    source_record = source_seed.record
                    prohibit_region = self._prohibited_region_participant(npc_id, source_record, 0.0, 0.3)
