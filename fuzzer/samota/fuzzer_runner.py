...
from fuzzer.samota.core.candidate import Candidate
from fuzzer.samota.models.ensemble import ensemble
from fuzzer.samota.models.RBF import Model as RBF_Model

from fuzzer.samota.core.mutator import ScenarioMutator
from fuzzer.samota.core.fitness import FitnessCalculator
...
class SAMOTAFuzzer(object):
    ...
    def __init__(
        ...
        self.no_of_Objectives = 2 # collision, reaching destination
        self.database = []
    ...
    def _record_result(self, mutated_seed: SeedWrapper):
    ...
    def evaulate_population_with_archive(self, pop, already_executed):
        ...
        for candidate in pop:
            if isinstance(candidate, Candidate):
                ...
                pure_seed = copy.deepcopy(self.refer_seed)
                mutated_seed = self.scenario_mutator.get_scenario(pure_seed, candidate_value)
                ...
                fitness_collision, fitness_dest, fitness_union = self.fitness_calculator.calculate(mutated_seed)
                mutated_seed.update_oracle(oracle=mr_oracle)
                mutated_seed.update_fitness(fitness_union)
                mutated_seed.save()
    ...
    def run_search(self, start_time, criteria, archive, g_max):
        ...
        while True:
            ...
            logger.error('Mutation GS Spend Time: [=]{}[=]', (m_end_time - m_start_time).total_seconds())
            if self._terminal_check(start_time, self.run_hour):
                return
            ...
            if self._terminal_check(start_time, self.run_hour):
                return
            m_end_time = datetime.now()
            logger.error('Mutation LS Spend Time: [=]{}[=]', (m_end_time - m_start_time).total_seconds())
    ...
    def run(self):
        ...
        logger.info('===== Start Fuzzer (Baseline SAMOTA) =====')
        archive = []
        threshold_criteria = [0, 0]
        g_max = 200
        self.minimize(start_time, threshold_criteria, archive, g_max)
