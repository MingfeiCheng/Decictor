...
class FeatureNet(object):
    ...
    def forward(self, scenario: SeedWrapperClass, resample='linear'):
        # use attributes: heading, speed, acceleration
        scenario_record = scenario.record.ego

        # obtain attributes and time
        x = []
        t = []
        for i in range(len(scenario_record.timestamps)):
            frame_attribute = scenario_record.attributes[i]
            x_f = [frame_attribute[2], frame_attribute[3], frame_attribute[4]]
            x.append(x_f)
            t.append(scenario_record.timestamps[i])
