...
class RegionCalculator:
    ...
    def preprocess(self, source_seed: SeedWrapper, motion_interval: float):
        ...
        self.time_list = [start_time + i * motion_interval for i in
                          range(int((end_time - start_time) // motion_interval) + 1)]
        ...
        for i, t in enumerate(self.time_list[:-1]):
            ...
            for npc_id in npc_ids:
                if npc_id == 'ego':
                    prohibit_region = self._prohibited_region_ego(self.adv_record, 0, 0.5)
                else:
                    source_record = source_seed.record
                    prohibit_region = self._prohibited_region_participant(npc_id, source_record, 0, 0.5)
