...
from fuzzer.decictor_wo_remove.core.operator.region_calculator import RegionCalculator
from fuzzer.decictor_wo_remove.core.operator.waypoint_searcher import WalkerWaypointSearcher
from scenario.config.lib.agent_type import Pedestrian
from scenario.config.section.walker import WAAgent

class WalkerOperator(object):
    ...
    def add_single_walker(self, source_seed: SeedWrapper, new_id: int) -> Optional[WAAgent]:

        last_waypoint = None
        waypoint_searcher = WalkerWaypointSearcher(source_seed.scenario.region, self.max_tries)
        ...
        for i, t in enumerate(time_list[:-1]):
            ...
            next_waypoint = waypoint_searcher.search(last_waypoint, delta_t, prohibit_region, Pedestrian())
        ...
        walker_id = new_id
        walker_agent = WAAgent(walker_id, mutable=True, route=route, agent_type=Pedestrian(), origin_trigger=trigger_time, noise_trigger=0.0)
        return walker_agent

    def remove_walkers(self, seed: SeedWrapper, num: int) -> Tuple[SeedWrapper, int]:
    ...
    def perturb_walkers(self, source_seed: SeedWrapper) -> Tuple[SeedWrapper, int]:
        ...
        if len(vehicle_ids) <= 0:
            ...
            for i in range(len(vehicle_ids)):
                ...
                if random.random() < self.perturb_prob:
                    perturbed_agent, is_perturb = self.perturb_single_walker(target_agent)
    ...
    def perturb_single_walker(self, agent: WAAgent) -> Tuple[WAAgent, bool]:
