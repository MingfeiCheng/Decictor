...
class ScenarioMutator:
    ...
    def _get_new_vehicle(self, seed: SeedWrapperClass) -> Optional[VDAgent]:
    ...
    def _mutation(self, seed: SeedWrapperClass) -> SeedWrapperClass:
        vds: List[VDAgent] = seed.scenario.vehicles.agents
        for i in range(len(vds)):
            if random.random() > 0.5:
                continue
    ...
    def _crossover(self, seed_i: SeedWrapperClass, seed_j: SeedWrapperClass):
        vds_i = seed_i.scenario.vehicles.agents
        vds_j = seed_j.scenario.vehicles.agents

        # change waypoints
        for i in range(len(vds_i)):
            if random.random() > 0.5:
                vd_ii: VDAgent = vds_i[i]
                vd_ji: VDAgent = vds_j[i]
                vd_ii.origin_trigger = vd_ji.origin_trigger
                vd_ii.route = copy.deepcopy(vd_ji.route)

                vds_i[i] = vd_ii

        seed_i.scenario.vehicles.agents = vds_i
        return seed_i

    def mutate(self, population: List[SeedWrapperClass], pc: float, pm: float):
    ...
    def get_initial_traffic(self, source_seed: SeedWrapperClass) -> SeedWrapperClass:
        mutated_seed = copy.deepcopy(source_seed)
        tries = 0
        while len(mutated_seed.scenario.vehicles.agents) < int(self.npc_number):
            tries += 1
            if tries > 100:
                break
            new_agent = self._get_new_vehicle(mutated_seed)
