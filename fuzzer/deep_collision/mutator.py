...
from fuzzer.random.core.operator.static_operator import StaticOperator
from fuzzer.random.core.operator.vehicle_operator import VehicleOperator
...
class RandomMutator:
    ...
    def __init__(self, cfg: DictConfig, adv_seed: SeedWrapper):
        ...
        self.operate_type = ['static', 'vehicle']
    ...
    def _remove_operator(self, next_seed: SeedWrapper, added_flag: bool) -> Tuple[SeedWrapper, bool]:
        ...
        if self.operate_type == 'static':
            ...
            # if existing_static_num > self.limit_static * 1.0 or not added_flag:
            #     target_count = max(int(existing_static_num * 0.4), 1) #int(random.uniform(self.min_remove_static, self.max_remove_static))
            # else:
            #     target_count = 0
            target_count = int(random.uniform(self.min_remove_static, self.max_remove_static))
            ...
            # else:
            #     target_count = 0
            target_count = int(random.uniform(self.min_remove_vehicle, self.max_remove_vehicle))
